...
set.seed(SEED)
knn <- train(biomassa~., data=df_train, method = "knn", tuneGrid = tuneGrid_knn)
knn
df_stats <- print_model_stats("KNN", df_test, target_var, knn, number_features, df_stats)

# RNA Hold-out
set.seed(SEED)
rna <- train(biomassa~., data=df_train, method = "nnet", linout=T, trace = FALSE)
rna
df_stats <- print_model_stats("RNA hold-out", df_test, target_var, rna, number_features, df_stats)

# RNA grid search
grid_rna <- expand.grid(size = seq(from=1,to=45, by=10), decay=seq(from=0.1,to=0.9,by=0.3))
set.seed(SEED)
rna_grid <- train(biomassa~., data=df_train, method = "nnet", linout=T, trace = FALSE, trControl = ctrl, tuneGrid = grid_rna, MaxNWts=10000, maxit=2000)
rna_grid
df_stats <- print_model_stats("RNA CV grid search", df_test, target_var, rna_grid, number_features, df_stats)

# SVM
set.seed(SEED)
svm <- train(biomassa~., data = df_train, method = "svmRadial")
svm
df_stats <- print_model_stats("SVM hold-out", df_test, target_var, svm, number_features, df_stats)

# SVM CV
set.seed(SEED)
svm_cv <- train(biomassa~., data = df_train, method = "svmRadial", trControl = ctrl)
svm_cv
df_stats <- print_model_stats("SVM CV", df_test, target_var, svm_cv, number_features, df_stats)

# SVM CV Grid
grid_cv <- expand.grid(C=c(1,2,10,50,100), sigma=c(.01,.015,.2))
set.seed(SEED)
svm_cv_grid <- train(biomassa~., data = df_train, method = "svmRadial", trControl = ctrl, tuneGrid=grid_cv)
svm_cv_grid
df_stats <- print_model_stats("SVM CV grid search", df_test, target_var, svm_cv_grid, number_features, df_stats)

# Randon forest
set.seed(SEED)
rf <- train(biomassa~., data = df_train, method="rf")
rf
df_stats <- print_model_stats("RF hold-out", df_test, target_var, rf, number_features, df_stats)

# RF CV
set.seed(SEED)
rf_cv <- train(biomassa~., data = df_train, method="rf", trControl = ctrl)
rf_cv
df_stats <- print_model_stats("RF CV", df_test, target_var, rf_cv, number_features, df_stats)

# RF CV grid search
grid_rf = expand.grid(mtry=c(2,5,7,9))
set.seed(SEED)
rf_cv_grid <- train(biomassa~., data = df_train, method="rf", trControl = ctrl, tuneGrid = grid_rf)
rf_cv_grid
df_stats <- print_model_stats("RF CV grid search", df_test, target_var, rf_cv_grid, number_features, df_stats)
...
### Novas predições
new_data <- read.csv("base/05 - Biomassa/5 - Biomassa - Dados - Novos Casos.csv")
View(new_data)
